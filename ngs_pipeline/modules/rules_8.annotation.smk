if config.get('calling_mode', 'joint') == 'joint':
	...
	rule r8_1_strip_genotypes:
		...
		"""
		input:
			vcf = rules.r4_8_apply_vqsr_indel.output.vcf
		output:
	...
	rule r8_3_restore_genotypes:
		...
		input:
			...
			cohort    = rules.r4_8_apply_vqsr_indel.output.vcf
		output:
	...
	rule r8_2_vep_germline_joint:
		"""Base VEP rule reused via 'use rule' for somatic calling."""
		wildcard_constraints:
			sample = "|".join(ngs.GRM_SAMPLES)
		input:
			vcf = "results/{run}/germline/vcf/{sample}.vcf.gz"
		output:
			vcf = temp("results/{run}/germline/vcf/{sample}.sites.annotated.vcf.gz")
		params:
			singularity    = config['tools']['singularity'],
			assembly       = config['assembly'],
			ref            = config['references']['genome_fa'],
			...
			spliceai_indel = config['references']['vep_plugins_data'].get('SpliceAI_INDEL', '')
		benchmark:
			'results/{run}/benchmarks/germline/vcf/{sample}.anno_base.bm'
		log:
			'results/{run}/logs/germline/{sample}.annotation_base.log'
		threads: 16
		resources:
			mem_mb      = 16000,
			runtime_min = 2880
	...
	rule r8_4_vep_germline_individual:
		"""VEP annotation for individual germline calling mode."""
		wildcard_constraints:
			sample = "|".join(ngs.GRM_SAMPLES)
		input:
			vcf = "results/{run}/germline/vcf/{sample}.vcf.gz"
