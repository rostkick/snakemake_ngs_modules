rule r1_read_alignment:
	input: 
		fr = lambda wc: long_df.query(f"sample=='{wc.sample}' & lane=='{wc.lane}' & reads_orientation=='R1'")['fastq'],
		rr = lambda wc: long_df.query(f"sample=='{wc.sample}' & lane=='{wc.lane}' & reads_orientation=='R2'")['fastq']
	output: 
